...
export class LocalLlmService {
  ...
  private nativeCompletion: Promise<unknown> | null = null;
  ...
  async load(modelPath: string, projectorPath?: string | null): Promise<boolean> {
    ...
    this.loadPromise = this.loadInternal(modelPath, projectorPath).finally(() => {this.loadPromise = null;});
    return this.loadPromise;
  }

  private async loadInternal(modelPath: string, projectorPath?: string | null): Promise<boolean> {
    ...
    try {
      ...
      this.context = await llama.initLlama({
        ...
        // A visual turn is intentionally compact: at most 128 image tokens,
      ...
      if (requestedProjector) {
        const initialized = await this.context.initMultimodal({
          ...
          use_gpu: false,
          image_min_tokens: 64,
          // The bundled LFM2 preprocessor also uses this to size its warmup
          // graph, reducing peak allocation as well as image prefill.
  ...
  async complete(request: LocalLlmCompletionRequest): Promise<string> {
    ...
    if (this.releasePromise || !this.context || this.state !== 'ready') {
    ...
    try {
      ...
      await this.trackCompletion(this.context.clearCache(true));
      if (generation !== this.generationId || !this.context) {throw new Error('Local response was cancelled.');}
      const completionPromise = this.trackCompletion(this.context.completion({
  ...
  async completeVision(request: LocalVisionCompletionRequest): Promise<string> {
    await this.cancellationPromise;
    if (this.releasePromise || !this.context || !this.visionReady || this.state !== 'ready') {
    ...
    try {
      ...
      const completionPromise = this.trackCompletion(this.context.completion({
        ...
        n_predict: this.thermalThrottled ? Math.min(request.maxTokens, request.jsonSchema ? 96 : 72) : request.maxTokens,
        stop: ['<|endoftext|>', '<|im_end|>', '<|end_of_turn|>'],
      }, (chunk: { token?: string }) => {
   ...
   * cannot yield directly. We push tokens onto a buffer and the consumer
   * drains it with a bounded wait that also checks stalled-stream deadlines.
   */
  async *completeStream(request: LocalLlmCompletionRequest): AsyncGenerator<LocalLlmStreamChunk, void, void> {
    ...
    try {
      ...
      completionPromise.then(
        () => { done = true; },
        (err) => { done = true; errorToThrow = err; },
      );
      while (true) {
        if (generation !== this.generationId) {return;}
        ...
        // A bounded wait also checks deadlines when native callbacks stop.
        await new Promise<void>(resolve => setTimeout(resolve, 25));
      }
    } finally {
      if (generation === this.generationId && this.nativeCompletion) {await this.cancel();}
      this.lastCompletionEndedAt = Date.now();
      if (generation === this.generationId && this.context) {this.state = 'ready';}
    }
  }
  ...
  async cancel(): Promise<void> {
    ...
    this.cancellationPromise = (async () => {
      await ignoreNativeFailure(() => context.stopCompletion());
      // A stop request can return while the image encoder is still running.
      await this.nativeCompletion?.catch(() => {});
      if (this.context === context) {this.state = 'ready';}
    })().finally(() => {this.cancellationPromise = null;});
  ...
  private trackCompletion<T>(promise: Promise<T>): Promise<T> {
    const tracked = promise.finally(() => {
      if (this.nativeCompletion === tracked) {this.nativeCompletion = null;}
    });
    this.nativeCompletion = tracked;
    return tracked;
  }
}
